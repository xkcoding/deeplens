...
1. **index.md** — Content-first home page (NO hero layout):
\`\`\`markdown
# ${outline.project_name}

<p class="deeplens-home-summary">${outline.summary}</p>

<div class="deeplens-badges">
${outline.detected_stack.map((t) => `  <span class="deeplens-badge">${t}</span>`).join("\n")}
</div>

[Architecture Overview](/architecture) | [Getting Started](/domains/${outline.knowledge_graph[0]?.id ?? ""}/)

## Domains

<ul class="deeplens-domains">
${outline.knowledge_graph.map((d, i) => `  <li><a href="/domains/${d.id}/">${i + 1}. ${d.title}</a><span class="deeplens-domain-desc">${d.description}</span></li>`).join("\n")}
</ul>
