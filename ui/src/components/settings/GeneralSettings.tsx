...
export function GeneralSettings({ config, onSave, currentProject, sidecarPort }: GeneralSettingsProps) {
  ...
  const preferredPort = config.sidecar_port ?? "54321";
      ...
      <div className="space-y-1.5">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-1">
            <span className="text-[10px] text-neutral-400">Sidecar Port</span>
            <Input
              type="number"
              value={preferredPort}
              onChange={(e) => onSave("sidecar_port", e.target.value)}
              placeholder="54321"
              className="h-8 text-xs"
              min={1024}
              max={65535}
            />
            {sidecarPort && Number(preferredPort) !== sidecarPort && (
              <p className="text-[10px] text-warning">
                Running on {sidecarPort} (configured: {preferredPort})
              </p>
            )}
          </div>
          <div className="space-y-1">
        ...
        <p className="text-[10px] text-neutral-400">
          MCP Server connects via Sidecar port. If occupied by old process, auto-reclaims.
        </p>
        <p className="flex items-center gap-1 text-[10px] text-warning">
          <AlertCircle className="size-3" />
          Sidecar port changes require app restart
        </p>
      </div>
